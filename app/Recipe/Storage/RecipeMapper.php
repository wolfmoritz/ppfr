...
class RecipeMapper extends DataMapperAbstract
{
    ...
    public function save(DomainObjectAbstract $recipe)
    {
        ...
        $recipe->url = $this->cleanUrl($recipe->title);

        // Set prep time duration in ISO8601 format
        if ($time = $this->stringToSeconds($recipe->prep_time)) {
            $recipe->prep_time_iso = $this->timeToIso8601Duration($time);
        }

        // Set cook time duration in ISO8601 format
        if ($time = $this->stringToSeconds($recipe->cook_time)) {
            $recipe->cook_time_iso = $this->timeToIso8601Duration($time);
        }
     ...
     * @return string
     */
    private function timeToIso8601Duration($time)
    {
        $units = array(
            "Y" => 365 * 24 * 3600,
            "D" => 24 * 3600,
     ...
     */
    private function stringToSeconds($str = null)
    {
        $totalSeconds = 0;
